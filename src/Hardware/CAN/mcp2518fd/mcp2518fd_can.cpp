...
// Maximum number of data uint8_ts in message
...
*********************************************************************************************************/
uint8_t mcp2518fd::begin(uint32_t speedset, const uint8_t clockset) {
    //SPI.begin();
...
int8_t mcp2518fd::mcp2518fd_WriteWord(uint16_t address, uint32_t txd) 
{
    ...
    // Split word into 4 uint8_ts and add them to buffer
    for (i = 0; i < 4; i++) {
...
int8_t mcp2518fd::mcp2518fd_WriteHalfWord(uint16_t address, uint16_t txd) {
    ...
    // Split word into 2 uint8_ts and add them to buffer
    for (i = 0; i < 2; i++) {
        spiTransmitBuffer[i + 2] = (uint8_t)((txd >> (i * 8)) & 0xFF);
    }
...
bool *crcIsCorrect) {
    ...
    nBytes + 5; // first two uint8_ts for sending command & address, third for
    // size, last two uint8_ts for CRC
    int8_t spiTransferError = 0;
    ...
    // First three uint8_ts need to be command
...
uint16_t nWords) {
    ...
    for (i = 0; i < nWords; i++) {
        w.word = 0;
        for (j = 0; j < 4; j++, n++) {
            w.uint8_t[j] = spiReceiveBuffer[n];
        }
        rxd[i] = w.word;
    }
    return spiTransferError;
...
uint16_t nWords) {
    ...
    for (i = 0; i < nWords; i++) {
        w.word = txd[i];
        for (j = 0; j < 4; j++, n++) {
            spiTransmitBuffer[n] = w.uint8_t[j];
        }
    }
...
bool enable) 
{
    ...
    spiTransferError = mcp2518fd_WriteByte(a, fCtrl.uint8_t);
...
GPIO_PIN_MODE gpio1) {
    ...
    spiTransferError = mcp2518fd_ReadByte(a, &iocon.uint8_t[3]);
    if (spiTransferError) {
        return -1;
    }
    ...
    // Write
    spiTransferError = mcp2518fd_WriteByte(a, iocon.uint8_t[3]);
    if (spiTransferError) {
        return -2;
    }
...
CAN_TX_FIFO_EVENT flags) {
    ...
    spiTransferError = mcp2518fd_ReadByte(a, &ciFifoCon.uint8_t[0]);
    if (spiTransferError) {
        return -1;
    }

    // Modify
    ciFifoCon.uint8_t[0] |= (flags & CAN_TX_FIFO_ALL_EVENTS);

    // Write
    spiTransferError = mcp2518fd_WriteByte(a, ciFifoCon.uint8_t[0]);
    if (spiTransferError) {
        return -2;
    }
...
CAN_RX_FIFO_EVENT flags) {
    ...
    // Modify
    ciFifoCon.uint8_t[0] |= (flags & CAN_RX_FIFO_ALL_EVENTS);
...
CAN_TX_FIFO_EVENT *flags) {
    ...
    spiTransferError = mcp2518fd_ReadByte(a, &ciFifoSta.uint8_t[0]);
    if (spiTransferError) {
        return -1;
    }

    // Update data
    *flags = (CAN_TX_FIFO_EVENT)(ciFifoSta.uint8_t[0] & CAN_TX_FIFO_ALL_EVENTS);
...
CAN_ERROR_STATE *flags) {
    ...
    // Update data
    *tec = ciTrec.uint8_t[1];
    *rec = ciTrec.uint8_t[0];
    *flags = (CAN_ERROR_STATE)(ciTrec.uint8_t[2] & CAN_ERROR_ALL);
...
bool flush) {
    ...
    txBuffer[0] = txObj->uint8_t[0]; // not using 'for' to reduce no of instructions
    txBuffer[1] = txObj->uint8_t[1];
    txBuffer[2] = txObj->uint8_t[2];
    txBuffer[3] = txObj->uint8_t[3];

    txBuffer[4] = txObj->uint8_t[4];
    txBuffer[5] = txObj->uint8_t[5];
    txBuffer[6] = txObj->uint8_t[6];
    txBuffer[7] = txObj->uint8_t[7];
    ...
    // Make sure we write a multiple of 4 uint8_ts to RAM
    uint16_t n = 0;
    uint8_t j = 0;

    if (txdNumBytes % 4) {
        // Need to add uint8_ts
        n = 4 - (txdNumBytes % 4);
        i = txdNumBytes + 8;
...
CAN_RX_FIFO_EVENT *flags) {
    ...
    // Update data
    *flags = (CAN_RX_FIFO_EVENT)(ciFifoSta.uint8_t[0] & CAN_RX_FIFO_ALL_EVENTS);
    return spiTransferError;
}
...
uint8_t *rxd, uint8_t nBytes) {
    ...
    // Number of uint8_ts to read
    n = nBytes + 8; // Add 8 header uint8_ts

    if (ciFifoCon.rxBF.RxTimeStampEnable) {
        n += 4; // Add 4 time stamp uint8_ts
    }

    // Make sure we read a multiple of 4 uint8_ts from RAM
    if (n % 4) {
        n = n + 4 - (n % 4);
    }
    ...
    myReg.uint8_t[0] = ba[0];
    myReg.uint8_t[1] = ba[1];
    myReg.uint8_t[2] = ba[2];
    myReg.uint8_t[3] = ba[3];
    rxObj->word[0] = myReg.word;

    myReg.uint8_t[0] = ba[4];
    myReg.uint8_t[1] = ba[5];
    myReg.uint8_t[2] = ba[6];
    myReg.uint8_t[3] = ba[7];
    rxObj->word[1] = myReg.word;

    if (ciFifoCon.rxBF.RxTimeStampEnable) {
        myReg.uint8_t[0] = ba[8];
        myReg.uint8_t[1] = ba[9];
        myReg.uint8_t[2] = ba[10];
        myReg.uint8_t[3] = ba[11];
...
int8_t mcp2518fd::mcp2518fd_ReceiveChannelUpdate(CAN_FIFO_CHANNEL channel) {
    ...
    // Write uint8_t
    spiTransferError = mcp2518fd_WriteByte(a, ciFifoCon.uint8_t[1]);
...
CAN_RX_FIFO_STATUS *status) {
    ...
    // Update data
    *status = (CAN_RX_FIFO_STATUS)(ciFifoSta.uint8_t[0] & 0x0F);
...
int8_t mcp2518fd::mcp2518fd_TransmitChannelEventAttemptClear(CAN_FIFO_CHANNEL channel)
{
    ...
    // Modify
    ciFifoSta.uint8_t[0] &= ~CAN_TX_FIFO_ATTEMPTS_EXHAUSTED_EVENT;

    // Write
    spiTransferError = mcp2518fd_WriteByte(a, ciFifoSta.uint8_t[0]);
    if (spiTransferError) {
        return -2;
    }
...
*********************************************************************************************************/
uint8_t mcp2518fd::mcp2518fd_sendMsg(const uint8_t *buf, uint8_t len, unsigned long id,
...
*********************************************************************************************************/
uint8_t mcp2518fd::mcp2518fd_init(uint32_t speedset, const uint8_t clock) {
    // Reset device
    mcp2518fd_reset();
...
//
uint8_t mcp2518fd::CANFDSPI_FilterDisable(CAN_FILTER filter)
{
    uint16_t a;
    REG_CiFLTCON_BYTE fCtrl;
    int8_t spiTransferError = 0;
    ...
    spiTransferError = mcp2518fd_ReadByte(a, &fCtrl.uint8_t);
    if (spiTransferError) {
        return -1;
    }
...
*/
uint8_t mcp2518fd::init_Filt_Mask(uint8_t num, uint8_t ext, unsigned long f, unsigned long m)
{
...
*********************************************************************************************************/
void mcp2518fd::setSleepWakeup(const uint8_t enable) {
    if (enable) {
...
*********************************************************************************************************/
uint8_t mcp2518fd::__setMode(const uint8_t opMode) {
...
*********************************************************************************************************/
uint8_t mcp2518fd::readMsgBufID(uint8_t status, volatile unsigned long *id,
volatile uint8_t *ext, volatile uint8_t *rtr,
...
*********************************************************************************************************/
uint8_t mcp2518fd::checkReceive(void) {
    CAN_RX_FIFO_STATUS status;
...
*********************************************************************************************************/
uint8_t mcp2518fd::checkError(uint8_t* err_ptr) {
    CAN_ERROR_STATE flags;
...
// **                          Status has to be read with readRxTxStatus.
// *********************************************************************************************************/
uint8_t mcp2518fd::mcp2518fd_readMsgBufID(volatile uint8_t *len, volatile uint8_t *buf) {
...
*free buffer.
*********************************************************************************************************/
uint8_t mcp2518fd::trySendMsgBuf(unsigned long id, uint8_t ext, uint8_t rtr, uint8_t len,
...
*change state.
*********************************************************************************************************/
void mcp2518fd::clearBufferTransmitIfFlags(uint8_t flags) {
...
*********************************************************************************************************/
uint8_t mcp2518fd::sendMsgBuf(uint8_t status, unsigned long id, uint8_t ext, uint8_t rtr,
...
*********************************************************************************************************/
uint8_t mcp2518fd::sendMsgBuf(unsigned long id, uint8_t ext, uint8_t rtr, uint8_t len,
...
*********************************************************************************************************/
uint8_t mcp2518fd::readRxTxStatus(void) {
    uint8_t ret;
    mcp2518fd_ReceiveChannelEventGet(APP_RX_FIFO, &rxFlags);
    ret = (uint8_t)rxFlags;
    return ret;
...
*********************************************************************************************************/
uint8_t mcp2518fd::checkClearRxStatus(uint8_t *status) {
    return 1;
}
...
**                          with one single readRxTxStatus call.
*********************************************************************************************************/
uint8_t mcp2518fd::checkClearTxStatus(uint8_t *status, uint8_t iTxBuf) {
    (void)iTxBuf;
    return 1;
}
...
*or input
*********************************************************************************************************/
bool mcp2518fd::mcpPinMode(const uint8_t pin, const uint8_t mode) {
    ...
    mcp2518fd_ReadByte(a, &iocon.uint8_t[3]);
    ...
    // Write
    mcp2518fd_WriteByte(a, iocon.uint8_t[3]);
...
*********************************************************************************************************/
bool mcp2518fd::mcpDigitalWrite(const uint8_t pin, const uint8_t mode) {
    int8_t spiTransferError = 0;
    ...
    spiTransferError = mcp2518fd_ReadByte(a, &iocon.uint8_t[1]);
    if (spiTransferError) {
        return -1;
    }
    ...
    // Write
    spiTransferError = mcp2518fd_WriteByte(a, iocon.uint8_t[1]);
    if (spiTransferError) {
        return -2;
    }
...
*********************************************************************************************************/
uint8_t mcp2518fd::mcpDigitalRead(const uint8_t pin) {
    ...
    mcp2518fd_ReadByte(cREGADDR_IOCON + 2, &iocon.uint8_t[2]);
...
uint8_t CANFD::dlc2len(uint8_t dlc) {
    if (dlc <= CAN_DLC_8)
    return dlc;
    switch (dlc) {
...
uint8_t CANFD::len2dlc(uint8_t len) {
    if (len <= CAN_DLC_8)
    return len;
