...
//! Reverse order of bits in uint8_t
...
class mcp2518fd : public MCP_CAN_1 {
    public:
    mcp2518fd(uint8_t _CS) : MCP_CAN_1(_CS), nReservedTx(0){};

    uint8_t spi_readwrite(uint8_t data) {
        uint8_t src[1] = {data};
        uint8_t dst[1];

        spi_write_read_blocking(this->pSPI, src, dst, 1);
   ...
   */
    virtual uint8_t begin(uint32_t speedset,
    const uint8_t clockset = MCP2518FD_20MHz); // init can
    
    bool __flgFDF = 0;                      // 1: FD, 0: 2.0

    virtual uint8_t CANFDSPI_FilterDisable(CAN_FILTER filter);

    virtual uint8_t init_Filt_Mask(uint8_t num, uint8_t ext, unsigned long f, unsigned long m);

    virtual void setSleepWakeup(const uint8_t enable);
    virtual uint8_t sleep();
    virtual uint8_t wake();
    virtual uint8_t __setMode(const uint8_t opMode);

    virtual uint8_t setMode(const uint8_t opMode)
    {
    ...
    virtual uint8_t checkError(uint8_t* err_ptr = NULL);

  /* ---- receiving ---- */
    virtual uint8_t checkReceive(void);
    virtual uint8_t readMsgBufID(uint8_t status, volatile unsigned long *id,
    volatile uint8_t *ext, volatile uint8_t *rtr,
    volatile uint8_t *len,
    volatile uint8_t *buf); // read buf with object ID
    /* wrapper */
   ...
   *       CAN_DLC_0..CAN_DLC_8(0..8) = data uint8_ts count, compatible to MCP2515 APIs
   *       CAN_DLC_12(9)              = 12 uint8_ts
   *       CAN_DLC_16(10)             = 16 uint8_ts
   *       CAN_DLC_20(11)             = 20 uint8_ts
   *       CAN_DLC_24(12)             = 24 uint8_ts
   *       CAN_DLC_32(13)             = 32 uint8_ts
   *       CAN_DLC_48(14)             = 48 uint8_ts
   *       CAN_DLC_64(15)             = 64 uint8_ts
   */
    virtual uint8_t trySendMsgBuf(unsigned long id, uint8_t ext, uint8_t rtr, uint8_t dlc,
    const uint8_t *buf, uint8_t iTxBuf = 0xff);
    virtual uint8_t sendMsgBuf(uint8_t status, unsigned long id, uint8_t ext, uint8_t rtr,
    uint8_t dlc, volatile const uint8_t *buf);
    virtual uint8_t sendMsgBuf(unsigned long id, uint8_t ext, uint8_t rtr, uint8_t dlc,
    const uint8_t *buf, bool wait_sent = true);
  /* wrapper */
    ...
    virtual void clearBufferTransmitIfFlags(uint8_t flags = 0);
    virtual uint8_t readRxTxStatus(void);
    virtual uint8_t checkClearRxStatus(uint8_t *status);
    virtual uint8_t checkClearTxStatus(uint8_t *status, uint8_t iTxBuf = 0xff);
    virtual bool mcpPinMode(const uint8_t pin, const uint8_t mode);
    virtual bool mcpDigitalWrite(const uint8_t pin, const uint8_t mode);
    virtual uint8_t mcpDigitalRead(const uint8_t pin);

    private:
    uint8_t mcp2518fd_readMsgBufID(volatile uint8_t *len, volatile uint8_t *buf);
    uint8_t mcp2518fd_sendMsg(const uint8_t *buf, uint8_t len, unsigned long id, uint8_t ext,
    uint8_t rtr, bool wait_sent);
    int8_t mcp2518fd_receiveMsg();

    private:
    uint8_t mcp2518fd_init(uint32_t speedset, const uint8_t clock); // mcp2518fdinit
